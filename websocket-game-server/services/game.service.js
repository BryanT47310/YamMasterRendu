...
const GameService = {
    ...
    send: {
        forPlayer: {
            ...
            gameScore: (playerKey, gameState) => {
                const playerScore = gameState.player1Score;
                const opponentScore = gameState.player2Score;
    ...
    grid: {
        ...
        checkAndScoreLines: (grid, playerKey) => {
            const directions = [
                { x: 0, y: 1 },  // vertical
                { x: 1, y: 0 },  // horizontal
            ...
            let victory = false;
            let vainqueur = null;
            let ownedCellsCount = 0; // Compteur des cases possédées par le joueur

            const countConsecutive = (x, y, dx, dy, owner) => {
                let length = 0;
                while (x >= 0 && x < grid.length && y >= 0 && y < grid[x].length && grid[x][y].owner === owner) {
                    length++;
                    x += dx;
                    y += dy;
                }
                ...
                row.forEach((cell, colIndex) => {
                    if (cell.owner === playerKey) {
                        ...
                        directions.forEach(direction => {
                            const length = countConsecutive(rowIndex, colIndex, direction.x, direction.y, playerKey);
                            if (length === 3) score += 1;
                            ...
                            /*if (victory) {
                                console.log('Victoire détectée pour le joueur ${playerKey}');
                            }
                            else console.log('Victoire pas détectée pour le joueur ${playerKey}');*/
                        });
                    }
                });
            });
            // Si le joueur a atteint le nombre maximum de cases possédées, on déclare la fin de partie
            if (ownedCellsCount >= 12) {
                victory = true;
            }
            return { score, victory, vainqueur };
        }
    },
